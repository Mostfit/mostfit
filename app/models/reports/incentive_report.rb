...
#  attr_accessor :from_date, :to_date
  ...
  def calc
    ...
    from_date_last = Date.new(Date.parse(from_date).year, (Date.parse(from_date).month) - 1, 1).strftime('%Y-%m-%d')  
    to_date_last = Date.new(Date.parse(to_date).year, (Date.parse(to_date).month) -1, -1).strftime('%Y-%m-%d')
