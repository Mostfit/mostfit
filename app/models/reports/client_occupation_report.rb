class ClientOccupationReport < Report
  ...
  def generate
    repository.adapter.query("select count(*) clients, o.name occupation, count(l.id) loans, sum(l.amount) amount from clients LEFT OUTER JOIN loans l ON l.id=clients.id LEFT OUTER JOIN occupations o ON clients.occupation_id=o.id group by clients.occupation_id")
  end
end
