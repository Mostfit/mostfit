#This report is for calculating the targets assigned to staff members on monthly basis filterd by Area.
class AreaTargetReport < Report
  attr_accessor :area_id, :area, :to_date

  validates_present :area_id

  def initialize(params, dates, user)
    @to_date = (dates and dates[:to_date]) ? dates[:to_date] : Date.today
    @from_date = Date.new(@to_date.year, @to_date.month, 01)
    @name = "Area Target Report from #{@from_date} to #{@to_date}"
    get_parameters(params, user)
  end

  def name
    "Area Target Report from #{@from_date} to #{@to_date}"
  end
  ...
  def generate
    ...
    @area[0].branches.centers.map {|center|
