class DebitAccountRule
  ...
  def amount(date)
    ...
    when :disbursement
      ...
      amount = amount + (Posting.all(:account => self.debit_account, "journal.date" => date, :amount.lt => 0).aggregate(:amount.sum) || 0)
    ...
    when :advance_principal
      ...
      amount = amount + (Posting.all("journal.date" => date, :account => self.debit_account, :amount.lt => 0).aggregate(:amount.sum) || 0)
      [(amount > 0 ? amount : 0), true]
