...
class LoanAggregatingCache
  ...
  # property :branch_id,                       Integer, :index => true  # these should be on subclassed models right?
  ...
  COLS= AGGR_BY_LAST = [:scheduled_outstanding_principal, :scheduled_outstanding_total,
  ...
  FLOW_COLS= AGGR_BY_SUM  = [:principal_due,              :principal_paid,
...
# what is the rationale for not caching these two methods like the rest?
  ...
  # def noop  # never used and look rather useless?  should it not be a class method on Kernel instead?
  #   0
  # end


# it does not seem this staling thing is implemented yet -- what is the idea with it?
...
# if this works we can make a self.consolidate(ids_array)
  ...
  def consolidate(other)
    ...
    # raise ArgumentError "cannot add cacher to something that is not a cacher" unless other.is_a? Cacher  # #is_a? is not very cheap.  better not use here.
...
#    raise ArgumentError "cannot add cachers of different classes" unless self.class == other.class   ### why is this commented out?
    ...
    attrs.merge!({ :date => date,    :id => -1,         :model_name => "Sum",  ### what is this doing here?  because you can sum a branchcache and a funderlinecache?  what use case?
