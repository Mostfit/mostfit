...
Mostfit::Business::Rules.prepare do |rule|
  rule.allow :name => :number_of_branches_in_area,  :model => Branch, :on => :create, :condition => [["area.branches.count", :less_than_equal,  2]]

  rule.allow :name => :loans_more_than_10k,         :model => Loan,   :on => :save,
             :precondition => [[:disbursal_date, :not, nil], [:amount, :greater_than_equal, 10000]], 
             :condition => [[:disbursed_by, :equal, "client.center.branch.manager"]]

  rule.reject :name => :max_centers_in_area,        :model => Center, :on => :create, 
              :condition => [["branch.area.branches.centers.count", :greater_than_equal, 100]]
  ...
  rule.reject :name => :amount_under_a_branch,      :model => Loan, :on => :save, :precondition => [[:disbursal_date, :not_equal, nil]], 
              :condition => [["client.center.branch.centers.clients.loans.sum(:amount)", :less_than_equal, 1000000]]
  rule.reject :name => :number_of_loans_per_day,    :model => Loan, :on => :save, :precondition => [[:disbursal_date, :not_equal, nil]],
              :condition => [["client.center.branch.centers.clients.loans.sum(:amount)", :less_than_equal, 1000000]]
end
