...
Mostfit::Business::Rule.prepare do |rule|
  ...
  rule.allow :name => :loans_more_than_10k,         :model => Loan,   :on => :save,   :if => {:disbursal_date.not => nil, :amount.gt => 10000}, 
             :condition => {:disbursed_by => "client.center.branch.manager"}
  ...
  rule.reject :name => :amount_under_a_branch,      :model => Loan, :on => :save, :if => {:disbursal_date.not => nil}, 
              :condition => ["client.center.branch.centers.clients.loans.sum(:amount)", :less_than_equal, 1000000]
  rule.reject :name => :number_of_loans_per_day,    :model => Loan, :on => :save, :if => {:disbursal_date.not => nil},
              :condition => ["client.center.branch.centers.clients.loans.sum(:amount)", :less_than_equal, 1000000]
end
