...
Merb::Router.prepare do
  ...
  match('/api/v1/regions/:id.xml').to(:controller => 'regions', :action =>'show', :format => 'xml')
  match('/api/v1/areas.xml').to(:controller => 'areas', :action =>'index', :format => 'xml')
  match('/api/v1/areas/:id.xml').to(:controller => 'areas', :action =>'show', :format => 'xml')
  match('/api/v1/branches.xml').to(:controller => 'branches', :action =>'index', :format => 'xml')
  match('/api/v1/branches/:id.xml').to(:controller => 'branches', :action =>'show', :format => 'xml')
