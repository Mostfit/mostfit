...
describe User do
  ...
  before(:each) do
    	@user = User.new(:id=>10, :login=>"sparsh",:created_at=>"2002-11-23", :updated_at=>"2003-11-23", :admin=>true)
	end
	...
	it "should not have false in all permission fields" do
	@user.read_only_user=false
	@user.data_entry_operator=false
	@user.mis_manager=false
   	@user.admin=false	
	@user.should_not be_valid
