...
describe ModelObserver do
  ...
  before(:all) do
    ...
    @funding_line = Factory( :funding_line, :amount => 10_000_000, :interest_rate => 0.15, :purpose => "for women", :disbursal_date => "2006-02-02", :first_payment_date => "2007-05-05", :last_payment_date => "2009-03-03", :funder => @funder)
    ...
    @organization = Factory(:organization, :name => "Organization", :org_guid => "63416690-b6c1-012e-8195-002170a9c469")
    @organization.save
    @organization.should be_valid
    
    @accounting_period = Factory(:accounting_period, :name => "Organization", :begin_date => (@date - 100), :end_date => (Date.today + 100), :organization_id => Organization.first.id)
    @accounting_period.save!


    @region = Factory(:region, :id => 1, :name => "Region 1", :manager_id => @manager.id, :creation_date => Date.today)
    ...
    @staff_member = Factory(:staff_member, :name => 'Bhagubhai Dholakia')
    @staff_member.errors.each{|e| puts e}
    @staff_member.should be_valid


    @branch = Factory(:branch, :name => "Bhavnagar", :manager => @manager, :code => 'BVN')
    @branch.errors.each {|e| puts e}
    @branch.should be_valid

    @center = Factory(:center, :name => "Munnar hill center", :manager => @manager, :branch => @branch, :creation_date => @date + 30, :meeting_day => :monday )
    @center.errors.each{|e| puts e}
    @center.should be_valid
    
    @client_type = Factory(:client_type, :type => 'Standard Client')
  ...
  it "should create an entry in the model event log when a client is created" do
    @client = Factory.build(:client, :name => 'HetalBen', :reference => 'GJ046921', :created_by_user_id => @user.id, :created_by_staff_member_id => @staff_member.id, :center => @center, :date_joined => (@date + 60), :client_type_id => @client_type.id )
    @client.save
...
# This one was giving an error "DefaultLoan" constant unknown. Not sure how to fix it.
