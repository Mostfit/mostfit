...
describe "resource(:staff_members)", :given => "an admin user" do
  ...
  describe "a successful POST", :given => "an admin user" do
    before(:each) do
      ...
      @response = request(resource(:staff_members), :method => "POST", :params => { :staff_member => { :name => "Test Staff Member" }})
    end
...
describe "resource(:staff_members)" do 
  describe "a successful DELETE", :given => "an admin user" do
    ...
    before(:each) do
      @response = request(resource(:staff_members), :method => "DELETE")
    end
...
# describe "resource(:staff_members, :edit)", :given => "an admin user" do
#   before(:all) do
#     load_fixtures :staff_members
#   end
#   before(:each) do
...
describe "resource(:staff_members)", :given => "an admin user" do
  ...
  describe "PUT", :given => "an admin user" do
    before(:each) do
      ...
      @response = request(resource(:staff_members), :method => "PUT", :params => { :staff_member => {:name => "abcde", :active => true} })
    end
