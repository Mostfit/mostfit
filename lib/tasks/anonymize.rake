...
namespace :mostfit do
  namespace :data do
    ...
    task :anonymize do
      ...
      models_to_anonymise = [Region, Area, Branch, Center, ClientGroup, Client, LoanProduct, StaffMember, Funder]
      ...
      fields_to_wipe_strings = [:name, :address, :contact_number, :landmark, :reference, :spouse_name, :mobile_number, :father_name]
      ...
      models_to_anonymise.each do |model|
        ...
        fields_to_wipe_strings.each do |field|
          if (instance.respond_to?(field))
            col_name = field.to_s; 
            col_value_stem = fields_to_wipe_strings_suggestions[field] ? "#{fields_to_wipe_strings_suggestions[field]}" : "#{field}"
